...
export function StockPurchaseDialog({
  ...
  const updatePurchase = useUpdateStockPurchase();
  ...
  const [stockGroups, setStockGroups] = useState<StockGroupDraft[]>([
    makeStockGroup(),
  ]);

  const [sharedCosts, setSharedCosts] = useState<SharedCostDraft[]>([
    makeTransportCost(),
  ]);
  ...
  useEffect(() => {
    ...
    if (purchase) {
      ...
      setStockGroups(
        purchase.stockGroups.map((group) => ({
          id: makeId(),
          category: group.category,
          quantity: group.quantity,
          unitBuyingPrice: group.unitBuyingPrice,
      ...
      if (purchase.amountPaid <= 0) {
        ...
        setPartialAmount(0);
      } else if (purchase.amountPaid >= purchase.totalCost) {
        setPaymentMode("full");
        setPartialAmount(purchase.amountPaid);
      } else {
    ...
    setStockGroups([makeStockGroup()]);

    setSharedCosts([makeTransportCost()]);
  ...
  const totalQuantity = useMemo(
    () => stockGroups.reduce((sum, group) => sum + (group.quantity || 0), 0),
    [stockGroups],
  );
  ...
  const sharedCostsTotal = useMemo(
    () =>
      roundMoney(
        sharedCosts.reduce((sum, cost) => sum + (cost.amount || 0), 0),
      ),
    [sharedCosts],
  );
  ...
  const supplierBalance = Math.max(0, roundMoney(totalCost - amountPaid));

  const addStockGroup = () => {
    setStockGroups((current) => [...current, makeStockGroup()]);
  };

  const removeStockGroup = (id: string) => {
    setStockGroups((current) => current.filter((group) => group.id !== id));
  };

  const updateStockGroup = (id: string, patch: Partial<StockGroupDraft>) => {
  ...
  const removeSharedCost = (id: string) => {
    setSharedCosts((current) => current.filter((cost) => cost.id !== id));
  };

  const updateSharedCost = (id: string, patch: Partial<SharedCostDraft>) => {
    setSharedCosts((current) =>
  ...
  const validate = () => {
    ...
    if (!supplierName.trim()) {
      toast({
        ...
        description: "Enter where the stock was bought.",
        variant: "destructive",
      });
    ...
    if (stockGroups.length === 0) {
      toast({
        title: "Add stock",
        description: "Add at least one stock group.",
        variant: "destructive",
      });
    ...
    for (const group of stockGroups) {
      ...
      if (
        !Number.isFinite(group.unitBuyingPrice) ||
        group.unitBuyingPrice <= 0
      ) {
        toast({
          title: "Enter the buying price",
          description:
            "Each stock group needs a buying price per item greater than zero.",
          variant: "destructive",
        });
    ...
    for (const cost of sharedCosts) {
      if (cost.amount < 0) {
        toast({
          title: "Additional costs cannot be negative",
          variant: "destructive",
        });
      ...
      if (cost.amount > 0 && !cost.label.trim()) {
        toast({
          ...
          description: "For example Transport, Packaging or Loading.",
          variant: "destructive",
        });
    ...
    if (!isEditing && paymentMode === "partial" && partialAmount <= 0) {
      toast({
        title: "Enter amount paid",
        description: "Enter the amount that was actually paid.",
        variant: "destructive",
      });
    ...
    if (!isEditing && paymentMode === "partial" && partialAmount >= totalCost) {
      toast({
  ...
  const handleSubmit = () => {
        ...
        },
        {
          onSuccess: async (updatedPurchase) => {
            await Promise.all([
              ...
              queryClient.invalidateQueries({
                queryKey: ["/api/stock-purchases", purchase.id],
              }),
            ...
            toast({
              ...
              description: `${updatedPurchase.purchaseNumber ?? "Purchase"} saved successfully.`,
            });
    ...
    createPurchase.mutate(
      ...
      {
        onSuccess: async (createdPurchase) => {
          await Promise.all([
            queryClient.invalidateQueries({
              queryKey: ["/api/stock-purchases"],
            }),
          ...
          toast({
            ...
            description:
              createdPurchase.supplierBalance > 0
      ...
      >
        ...
        <div className="space-y-6 px-4 py-5 sm:px-6">
          ...
          <section className="space-y-3">
            <div>
              ...
              <p className="mt-0.5 text-xs text-muted-foreground">
                When and where the stock was bought.
              </p>
            </div>
            ...
            <details className="rounded-xl border bg-muted/10">
              <summary className="cursor-pointer px-3.5 py-3 text-xs font-medium text-muted-foreground">
                More purchase details (optional)
              </summary>
          ...
          <section className="space-y-3">
            <div className="flex items-end justify-between gap-3">
              <div>
                <h3 className="text-sm font-semibold">Stock bought</h3>

                <p className="mt-0.5 text-xs text-muted-foreground">
                  Group the stock instead of entering every product.
                </p>
              </div>
            ...
            <div className="space-y-2">
              {stockGroups.map((group, index) => (
                <div key={group.id} className="rounded-xl border bg-card p-3">
                  <div className="flex items-end gap-2">
                    ...
                    <Button
                      ...
                      variant="ghost"
                      size="icon"
                      disabled={stockGroups.length === 1}
                      onClick={() => removeStockGroup(group.id)}
                      className="h-10 w-9 shrink-0 text-muted-foreground hover:text-destructive"
                      aria-label={`Remove stock group ${index + 1}`}
                    >
                      <Trash2 className="h-4 w-4" />
                    </Button>
                  </div>

                  <div className="mt-2 grid grid-cols-[90px_minmax(0,1fr)] gap-2">
                    <div className="space-y-1.5">
                      <label className="text-[11px] text-muted-foreground">
                        Quantity
                      </label>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="relative">
                        ...
                        <Input
                          ...
                          min={0}
                          step="0.01"
                          value={group.unitBuyingPrice || ""}
                          onChange={(event) =>
                            updateStockGroup(group.id, {
                              unitBuyingPrice: Math.max(
                                0,
                  ...
                  <div className="mt-2 flex items-center justify-between rounded-md bg-muted/45 px-2.5 py-2">
                    ...
                    <span className="text-xs font-semibold tabular-nums">
                      {formatCurrency(
                        roundMoney(group.quantity * group.unitBuyingPrice),
                      )}
                    </span>
                  </div>
          ...
          <section className="space-y-2">
            <div>
              <h3 className="text-sm font-semibold">Stock cost</h3>

              <p className="mt-0.5 text-xs text-muted-foreground">
                Calculated from quantity × buying price for each stock group.
              </p>
            </div>

            <div className="flex items-center justify-between rounded-xl border bg-card px-3 py-3">
              <span className="text-xs text-muted-foreground">Goods total</span>
          ...
          <section className="space-y-3">
            <div className="flex items-end justify-between gap-3">
              <div>
                ...
                <p className="mt-0.5 text-xs text-muted-foreground">
                  Transport, packaging, loading or other costs.
                </p>
              </div>
              ...
              {sharedCosts.map((cost) => (
                ...
                >
                  ...
                  <Button
                    ...
                    variant="ghost"
                    size="icon"
                    onClick={() => removeSharedCost(cost.id)}
          ...
          {isEditing && purchase ? (
            <section className="space-y-3">
              <div>
                ...
                <p className="mt-0.5 text-xs text-muted-foreground">
                  Payments are managed separately so payment history stays
                  accurate.
                </p>
              </div>
            ...
            <section className="space-y-3">
              <div>
                ...
                <p className="mt-0.5 text-xs text-muted-foreground">
                  Any unpaid amount becomes supplier debt automatically.
                </p>
              </div>
          ...
          <section className="rounded-2xl bg-muted/50 p-4">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between gap-4">
                ...
                <span className="tabular-nums">
                  {formatCurrency(sharedCostsTotal)}
                </span>
              </div>
        ...
        <div className="sticky bottom-0 flex items-center gap-3 border-t bg-background/95 px-4 py-4 backdrop-blur sm:px-6">
          <Button
            ...
            disabled={
              isEditing ? updatePurchase.isPending : createPurchase.isPending
            }
